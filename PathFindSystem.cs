    ...
    public async Task<List<object[]>> FindPath(int[] _agent, Transform start, Transform end)
    {
        ...
        { 
            ...
            {  
                ...
                _startPosition = _chunkSystem.GetRelativePosition(Vector3Int.FloorToInt(start.position));
                _endPosition = _chunkSystem.GetRelativePosition(Vector3Int.FloorToInt(end.position)); 

                return await Task.Run(() => FindPathAsync(_agent, ChunkSystem._boolGridOrigin, ChunkSystem._boolGrid, _startPosition, _endPosition));
            } 
            return null;
        }
        finally
