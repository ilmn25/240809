         ...
         
{
    ...
    {   
        ...
        _chunkSystem = transform.parent.GetComponent<ChunkSystem>(); 
        ChunkSystem.PlayerChunkPositionUpdate += HandleChunkMapTraverse;
        _chunkSize = ChunkSystem.CHUNKSIZE;
        _chunkDepth = ChunkSystem.CHUNKDEPTH;
        _renderDistance = _chunkSystem.RENDER_DISTANCE;
    ...
    { 
        ...
        foreach (var kvp in _activeChunks)
        {
            ...
            if (traverseCheckPosition.x > ChunkSystem._chunkPosition.x + _renderDistance * _chunkSize || traverseCheckPosition.x < ChunkSystem._chunkPosition.x - _renderDistance * _chunkSize
                || traverseCheckPosition.z > ChunkSystem._chunkPosition.z + _renderDistance * _chunkSize || traverseCheckPosition.z < ChunkSystem._chunkPosition.z - _renderDistance * _chunkSize)
            {
                Destroy(kvp.Value);
        ...
        for (int x = -_renderDistance; x <= _renderDistance; x++)
        {
            for (int z = -_renderDistance; z <= _renderDistance; z++)
            {
                Vector3Int traverseCheckPosition = new Vector3Int(
                    ChunkSystem._chunkPosition.x + x * _chunkSize,
                    0,
                    ChunkSystem._chunkPosition.z + z * _chunkSize
                );
    ...
    { 
        ...
        { 
            if (replace || !_activeChunks.ContainsKey(chunkCoord))
            {
                ...
                (_chunkMap, _frontFace, _backFace, _leftFace, _rightFace, _corner) = _chunkSystem.LoadChunkMap(chunkCoord);
                if (_chunkMap != null)
                {
    ...
    ChunkYCullHandler _chunkYCullHandler;
    private void LoadMeshObject(bool replace = false)
    {
        ...
        if (!replace)
        {
            ...
            _meshRenderer.material = BlockSystem._meshMaterial; 

            _chunkYCullHandler = _meshObject.AddComponent<ChunkYCullHandler>();  
            _chunkYCullHandler._chunkMap = _chunkMap;
            _chunkYCullHandler._meshData = _mesh;
            _chunkYCullHandler._verticesShadow = _verticesShadow;
        ...
        else 
        {
            ...
            _chunkYCullHandler = _meshObject.GetComponent<ChunkYCullHandler>();  
            _chunkYCullHandler._chunkMap = _chunkMap;
            _chunkYCullHandler._meshData = _mesh;
            _chunkYCullHandler._verticesShadow = _verticesShadow;
            _chunkYCullHandler.HandleAssignment(); 
        } 
    }
    ...
    { 
        try
        {   
            NativeHashMap<int, Rect> _textureRectDictionary = new NativeHashMap<int, Rect>(BlockSystem._textureRectDictionary.Count, Allocator.TempJob);
            foreach (var kvp in BlockSystem._textureRectDictionary)
            {
            ...
            MeshMathJob job = new MeshMathJob
            {
                ...
                _textureAtlasWidth = BlockSystem._textureAtlasWidth,
                _textureAtlasHeight = BlockSystem._textureAtlasHeight, 
