...
public class EntityMovementHandler : MonoBehaviour
{
    ...
    { 
        if (transform.position.y < -1) transform.position = CustomLibrary.AddToVector(transform.position, 0, 40, 0);
    ...
    { 
        if (_isGrounded && _direction.y > 0 && _entityPathFindHandler._pointDistance < 2f)
        {
