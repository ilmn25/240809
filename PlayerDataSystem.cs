...
{ 
    ...
    public List<KeyValuePair<int, Item>> _playerInventory;
    ...
    public void AddItem(int id, int quantity = 1)
    {
        ...
        {
            Item newItem = ItemLoadSystem.itemList.Find(item => item.ID == id);
            if (newItem != null)
            {
                int slotID = GetSmallestAvailableSlotID();
                _playerInventory.Add(new KeyValuePair<int, Item>(
                    ...
                    new Item(
                        ...
                        newItem.UseTime,
                        newItem.Description,
                        newItem.IsConsumable,
                        newItem.CraftingMaterials
                    )
                ));
            }
        }
    ...
    private void LoadPlayerData()
    {
        ...
        if (File.Exists(filePath))
        {
            ...
            {
                _playerInventory = (List<KeyValuePair<int, Item>>)_bf.Deserialize(file);
            }
        }
        else
        {
...
{
    public List<KeyValuePair<int, Item>> inventory;

    public PlayerData(List<KeyValuePair<int, Item>> itemList)
    {
        inventory = itemList;
    }
}
