...
public static class EntitySync
{
    ...
    /// <summary>Queue a one-time spawn broadcast for a newly created structure.
    /// (Items use ItemSync's lean batch instead.)</summary>
    ...
    /// <summary>Tracks last received animation hash per uid so we know when to play a new one-shot.</summary>
    ...
    private static void OnBatchEntityInfoMessageReceived(BatchEntityInfoMessage message)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            ...
            // Entity might have been destroyed locally (pickup via F/right-click).
            // If Machine was nulled by Unload(), skip — the server will broadcast the destroy soon.
            if (targetInfo.Machine == null)
                continue;
            ...
            // Non-player entities are host-owned — skip any that slipped through.
    ...
    public static void SendBatch()
    {
        ...
        _batchAnimTriggers.Clear(); _batchAnimNormalizedTimes.Clear(); _batchOwnerIds.Clear();
        ...
        // Pending spawns: one-shot structures that only need a single broadcast to appear.
        // (Items use ItemSync's lean batch instead.)
        ...
        // Pending unloads (processed BEFORE the empty check so the last entity's
        // destroy is always broadcast to clients).
        ...
        // Send initial storage for NEW container entities (StorageSync only sends on
        // modification). Batch creates the entity; storage follows in-order.
