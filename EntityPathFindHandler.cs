...
public class EntityPathFindHandler : MonoBehaviour
{ 
    public bool _repathRoutine = false;
    private PathFindSystem _pathFindSystem;
    ...
    void Start()
    {
        ...
        _pathFindSystem = GameObject.Find("world_system").GetComponent<PathFindSystem>(); 
        ...
        if (!_moveOccupied)
        {
            ...
            // { 
                ...
                if (!_playerReached)
                {
                    ...
                    {  
                        await Task.Delay((int)(1500 / _entityMovementHandler.SPEED_WALK)); // Convert seconds to milliseconds
                        if (this != null && _nextPoint < _path.Count -2)
                        {
                            _nextPoint++;  
                            transform.position = CustomLibrary.AddToVector((Vector3)_path[_nextPoint][0], 0, 0.1f, 0);
                        } else return;
                    }   
                } else _entityMovementHandler._direction = Vector3.zero;
    ...
    public void HandlePathFindActive()
    {
        ...
        if (_updateEntityPosition && _entityMovementHandler._isGrounded)
        { 
            _selfPositionPrevious = transform.position;
            _updateEntityPosition = false; 
        }
        if (_updatePlayerPosition && _playerMovementSystem._isGrounded) 
        {
            _playerPositionPrevious = _player.transform.position;
            _updatePlayerPosition = false;
        }
        
        _entityMovementHandler._direction = Vector3.zero; 
        ...
        { 
            ...
            { 
                ...
                if (_entityMovementHandler._isGrounded && _pointDistance < NEXT_POINT_DISTANCE)
                {
                    _nextPoint++;
                } 
                ...
                _entityMovementHandler._direction = ((Vector3)_path[_nextPoint][0] - transform.position).normalized; 
            } else _entityMovementHandler._direction = (CustomLibrary.AddToVector(_player.transform.position, 0, -0.3f, 0) - transform.position).normalized;
    ...
    { 
        ...
        if (!_playerReached && !_isPathFinding)
        {
            ...
            if (_playerMovementSystem._isGrounded && _playerMoved)
            { 
                GetPath();  
    ...
    bool IsStuck()
    {
        if (_entityMovementHandler._isGrounded) 
        { 
    ...
    private async void GetPath()
    {
        ...
        {  
            ...
            _pathQueued = await _pathFindSystem.FindPath(AGENT, transform, _player.transform);  
            if (this != null)
            {
